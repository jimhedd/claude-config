...
4. **If a PR exists**: update the PR to match the latest commit:
   - Get the latest commit subject: `git log -1 --format=%s`
   - Get the latest commit body: `git log -1 --format=%b` (may be empty, that is fine)
   - Run `gh pr edit --title <title> --body <body>` using the values above.
   - If the edit fails, report the error and stop.
   - Report success and include the PR URL from step 3.
