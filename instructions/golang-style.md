...
Note: This guide was verbatim copied from https://raw.githubusercontent.com/uber-go/guide/master/style.md as
a starting point for a Doordash specific Go style guide. This guide will grow, shrink, and change as to the
needs fo Doordash. We may have started on the shoulders of giants, but we will forge our own path.

Editing this document:

- Discuss all changes in the Go Frameworks team first.
...
Dos and Don'ts of writing Go code at Doordash. These rules exist to keep the code
base manageable while still allowing engineers to use Go language features
productively.

This guide was originally created at Uber, and the platform team at Doordash is now
maintaining this fork as way to bring some colleagues up to speed with using Go.
Over the years it has been amended based on feedback from others.

This documents idiomatic conventions in Go code that we follow at Doordash. A lot
of these are general guidelines for Go, while others extend upon existing
resources:

1. [Effective Go](https://golang.org/doc/effective_go.html)
2. [Go Common Mistakes](https://github.com/golang/go/wiki/CommonMistakes)
3. [Go Code Review Comments](https://github.com/golang/go/wiki/CodeReviewComments)
4. [100 common mistakes in go and how to avoid them](https://100go.co/)
...
### Packages and Code Sharing

The underlying components of your structs (classes) should generally remain hidden from your end users as much as possible. Data classes that exist for serialization purposes are an exception due to the way Go handle serialization.
``` Go
...
```
In this example `someOtherField` is neither serialized nor deserialized (regardless of format protobuf/json/yaml/etc).

Only exporting interfaces is one way to ensure other packages only rely on your public behavior, allowing you to change underlying behavior without requiring widespread changes. This also will prevent other packages from modifying your internal state and potentially ending up in unsupported configurations.
...
type processBasedPool struct {
  ...
}

type wasmBasedPool struct {
  ...
}
...
func NewPool(config PoolConfig) (Pool, error) {
  switch config.Style {
    case ProcessBased:
      return newProcessBasedPool(config)
    case WasmBased:
      return newWasmBasedPool(config)
    default:
      return nil, fmt.Errorf("Unsupported Pool Style: %s", config.Style.String())
  }
}

func newProcessBasedPool(config PoolConfig) (Pool, error) {
  ...
}

func newWasmBasedPool(config PoolConfig) (Pool, error) {
  ...
}
```

Here the implementations of our Pools are all private, and the users just interact with the public interfaces, enabling us to refactor all of their internal details without requiring any client changes.

### Functional Programming

While Go is not a functional language, it is also not an Object Oriented language.
Following functional programming paradigms can lead to easier to read and maintain code.

By default, avoid reassigning variables or members of a struct after they have been created. Most fields SHOULD be immutable after creation. Unfortuantely Go has no way to enforce this, but its something to strive for in our code bases as it will make it far easier to understand where or how a given field was set if that assignment only happens in a single place.

### Function arguments and Pointers

All arguments in Go are pass by value. That value, however, can be a pointer to a struct. Passing in the pointer prevents a copy of the data from being made, and also allows modification of the fields inside it.

[For example](https://go.dev/play/p/vk-q72699W8):
``` Go
package main
...
Goodbye
```
since the `Value` is given a copy of the data, and hence modifications are not returned to the calling function, but when passing by pointer in the `Pointer` function, we see the modification.
...
> [!CAUTION]
> The use of the `go.uber.org/atomic` package is under review.
Regardless, having global variables that are to be protected via atomics is not
recommended. Atomics should be (at least) protected by being at most package
global, and the relevant functions to manipulate their values should be defined
close to the variable definition with appropriate commentary to ensure it is
abundantly clear which variables are protected in which manner.
...
Avoid lines of code that require readers to scroll horizontally or turn their
heads too much.

We recommend a soft line length limit of **99 characters**. Authors should aim
to wrap lines before hitting this limit, but it is not a hard limit. Code is
...
Note: This section has been removed (commented out for now). This flies in the
face of Go variable naming conventions and make for a clashing view when an
engineer comes to Go from other languages where a leading underscore indicates
a private symbol.
...
with recommended linters and settings. Note, this file will be incorporated
into the Doordash Go monorepo at some point in the future.
